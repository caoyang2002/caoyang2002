+++
title = 'Vim 命令速查'
date = 2026-01-26T13:35:50+08:00
draft = false
author = "simons"
categories = ["编程工具", "效率工具"]
tags = ["Vim", "编辑器", "快捷键", "命令速查", "文本编辑"]
description = "这是一份全面的 Vim 命令速查手册，涵盖光标移动、文本编辑、插入模式、查找替换、分屏窗口、缓冲区、宏录制等核心操作，并结合实际案例与技巧说明，帮助开发者快速掌握 Vim 的高效使用方法，提升编码与文本处理效率。"
+++
...
2. 组合 Shift 的大跨度移动  
   - `<S-Up>`   向上翻一页（Page-Up 效果）  
   - `<S-Down>`  向下翻一页（Page-Down 效果）  
   - `<S-Left>`  向左移动一个单词（word）  
   - `<S-Right>`  向右移动一个单词（word）  
...
────────────────────
使用提示  
• 所有 “Ctrl” 组合键在文档中写作 `<C-x>` 形式。  
• 若需记忆，可把 `<C-o>` 与 `<C-\><C-o>` 理解为 **“插入模式里的冒号命令”**；把 `<C-r>` 系列理解为 **“插入模式里的粘贴板”**。
## 2. 块级移动
...
注意：块级 移动要善于使用 Vim 的[书签](#vim书签)和[标签页](#标签页)功能，实现文件内容及文件之间快速移动。
...
`I`  →  Insert at BOL（行首）  
- 先把光标挪到当前行的**第一个非空白字符**；如果整行是空，就到行首。  
- 然后进入插入模式。  
- 场景：批量在行首加注释符号、缩进、#。  
...
`A`  →  Append at EOL（行尾）  
   • 一次性把光标送到行尾，再进入插入。  
   • 场景：写代码时在行末补 ;、写文本时在句尾加标点。  

`o`  →  open below  
- 在当前行**下方**新建一行空行，光标落在行首，进入插入。  
- 原光标所在行的内容不受影响。  
- 场景：快速写下一行代码、写列表的下一项。  

`O`  →  Open above  
- 在当前行**上方**新建一行空行，光标落在行首，进入插入。  
- 场景：想在当前代码前插一行日志、在标题前插空行。  

`s`  →  substitute（删字符并插入）  
- 删除光标下的**1 个字符**，然后进入插入。  
- 等价于 `x` 之后再 `i`，但一次完成。  
- 场景：打错一个字母，直接覆盖。  也可以使用 `r`

`S`  →  Substitute whole line  
- 删除当前**整行**（保留空行），光标落在行首，进入插入。  
- 等价于 `dd` 之后再 `O`。  
- 场景：整行重写、清空日志行。  

## 2. 插入模式的特殊操作

由 i, I, a, A, o, O, s, S 等命令进入插入模式后：
...
```
   ...
   - `<C-v> {char}`   插入字面量字符（如不可见字符）  
   - `<C-v> {code}`  以 3 位十进制 ASCII 码插入字符，例：`<C-v> 065` → A  
   - `<C-v> x41>`   以 2 位十六进制插入字符，例：`<C-v> x41` → A  
   - `<C-v> o101>`  以 3 位八进制插入字符，例：`<C-v> o101` → A  
   - `<C-v> u1234>`  以 4 位十六进制插入 Unicode，例：`<C-v> u4e2d` → 中  
   - `<C-v> U12345678>` 以 8 位十六进制插入 Unicode  
   - `<C-k> {ch1}{ch2}` 插入双字符组合（digraph）

Ctrl-v 数字输入的完整格式

| 前缀  | 进制   | 位数限制  | 举例               | 插入结果 |
| --- | ---- | ----- | ---------------- | ---- |
| 无前缀 | 十进制  | 1–3 位 | `<C-v>065`       | A    |
| `x` | 十六进制 | 1–2 位 | `<C-v>x41`       | A    |
| `o` | 八进制  | 1–3 位 | `<C-v>o101`      | A    |
| `u` | 十六进制 | 4 位   | `<C-v>u4e2d`     | 中    |
| `U` | 十六进制 | 8 位   | `<C-v>U0001f600` | 😀   |
...
- Esc：`<C-v><Esc>` （显示为 `^[`）  
- Tab：`<C-v><Tab>` （显示为 `^I`）  
   … 任何键都可以在 `<C-v>` 之后直接按，Vim 会原样插入其底层字节。

插入当前终端/编码下任意字节  

   在 UTF-8 终端里，`<C-v>` 后输入的字节序列会按 UTF-8 规则解析；在 Latin-1 终端里则按 Latin-1 解析。  
   因此可用 `<C-v>` 输入「不是合法 UTF-8」的单个字节（例如 0xFF），从而制造“损坏”的文本以测试 iconv 等工具。

`<C-k>` 双字符组合（digraph）速查
...
查看全部：普通模式 `:digraphs` 回车，或插入模式 `<C-k><F1>`（部分终端）。

寄存器与表达式插入的隐藏技巧

| 命令                     | 作用          | 举例                   |
| ---------------------- | ----------- | -------------------- |
| `<C-r>=expand("%:t")`  | 插入当前文件名     | 写注释时快速插文件名           |
| `<C-r>=line('.')`      | 插入当前行号      | 写 Markdown 行内锚点      |
| `<C-r>=strftime('%F')` | 插入今天日期      | 2025-08-24           |
| `<C-r>@:`              | 插入上一条 Ex 命令 | 写 commit message 时引用 |
| `<C-r>@/`              | 插入上一次搜索模式   | 写文档时引用正则             |

Ctrl-g 在插入模式里的小动作（很多人不知道）

| 快捷键                 | 作用                               |
| ------------------- | -------------------------------- |
| `<C-g>j` / `<C-g>k` | 在软换行（wrap）开启时，按“屏幕行”上下移动，不离开插入模式 |
| `<C-g>u`            | 手工产生一个 undo 断点；长行输入时避免整行都被一次撤销   |

删：  

```bash
<C-h> 字符  <C-w> 单词  <C-u> 行前
```
...
:[range]d           # 删除范围，例如 :20,30d 是删除（剪切） 20 到 30 行，
:10d				# 删除（剪切）第 10 行
...
```bash
f{char}             # 向后搜索当前行第一个为 {char} 的字符，（f: find），光标停留在目标字符上
F{char}             # 向前搜索当前行第一个为 {char} 的字符
t{char}             # 向后搜索当前行第一个为 {char} 的字符前（t: to），光标停留在目标字符前
T{char}             # 向前搜索当前行第一个为 {char} 的字符前，光标停留在目标字符前
```

### c.查找的快捷执行与撤销

```bash
<number>f{char}					#可以找到第 number 个为 char 字符，下同
...
普通（Normal）模式下的替换命令格式（substitute）。

```bash
:[range]s[ubstitute]/{pattern}/{string}/[flags]
```
对应的就是 Vim 的“查找替换”命令。各部分含义如下：
...
1. `[range]`（可选）——作用范围  
   ...
   - `10,20`：第 10～20 行  
   - `'<,'>`：上一次 **可视模式** 选中的区域  
   - `.` 当前行，`$` 最后一行，都可与其他符号组合，例如 `.,$` 表示从当前行到文件末尾。

2. `s` 或 `substitute`——命令本身  
   可以简写成 `:s`，也可以写全 `:substitute`。

3. `{pattern}`——要查找的正则表达式  
   例如 `foo`、`\d\+`、`^#.*` 等。

4. `{string}`——替换后的文本  
   可以直接写纯文本，也可以用 `\1`、`\2` 等引用正则里的捕获组。

5. `[flags]`（可选）——附加标志  
   常见标志：  
   - `g`（global）：一行里全部匹配都替换，不加则只替换每行第一个。  
   - `c`（confirm）：每次替换前询问确认。  
   - `i`（ignore case）：忽略大小写。  
   - `I`（no ignore case）：区分大小写。  
   - `n`（number）：只统计匹配数量，不真正替换。

### a. 常用示例

```vim
:%s/foo/bar/g          " 把全文件所有 foo 换成 bar
:10,20s/foo/bar/gc     " 第 10～20 行，把 foo 换成 bar，并逐个确认
:s/foo/bar/            " 仅替换当前行第一个 foo
:%s/\s\+$//g           " 删除行尾多余空白
```
...
- 分隔符 `/` 可换成其他字符（如 `#`、`@`），避免与模式里的 `/` 冲突：  
  `:%s#/usr/local#/opt#g`
- 如果要替换的内容里带 `/`，用 `\` 转义或者换分隔符会更方便。
...
四、临时切换 & 寄存器  
   - `<C-o>`    临时执行一次普通模式命令，随后立即返回插入模式  
   - `<C-\> <C-o>` 同上，但执行命令时光标不移动  
   - `<C-r> 0`   插入寄存器 0 的内容  
   - `<C-r> "`   插入匿名寄存器的内容  
   - `<C-r> =`   插入表达式的计算结果  
   - `<C-r> :`   插入上一次命令行命令  
...
![vim 命令键盘图](https://github.com/chloneda/vim-cheatsheet/raw/master/resources/vim-commands_CN.png)
